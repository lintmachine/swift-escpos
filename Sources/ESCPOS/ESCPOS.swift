...
public enum ESCPOS {
    ...
    public enum Command: Equatable, Sendable {
        ...
        case raw(Data)
        case ascii(String)
        indirect case group([ESCPOS.Command])
        ...
        public var dataValue: Data {
            ...
            case let .printAndFeedPaper(amount): return .init([.ESC, .J, amount])
            case let .printAndReverseFeedPaper(amount): return .init([.ESC, .K, amount])
            ...
            case let .underline(mode): return .init([.ESC, .HYPH, mode.rawValue])
            case let .emphasize(bool): return .init([.ESC, .E, bool ? 1 : 0])
            case let .doubleStrike(bool): return .init([.ESC, .G, bool ? 1 : 0])
            case let .characterFont(font): return .init([.ESC, .M, font.rawValue])
            case let .internationalCharacterSet(set): return .init([.ESC, .R, set.rawValue])
            case let .clockwise90DegreeRotation(rotation): return .init([.ESC, .V, rotation.rawValue])
            case let .printColor(color): return .init([.ESC, .r, color.rawValue])
            case let .characterCodeTable(table): return .init([.ESC, .t, table.rawValue])
            case let .upsidedown(bool): return .init([.ESC, .LBKT, bool ? 1 : 0])
            case let .characterSize(vertical, horizontal): return .init([.GS, .BANG, (vertical.rawValue << 4) | horizontal.rawValue])
            ...
            case let .selectPrintDirectionInPageMode(dir): return .init([.ESC, .T, dir.rawValue])
            case let .setPrintAreaInPageMode(x, y, w, h): return .init([.ESC, .W, x.low, x.high, y.low, y.high, w.low, w.high, h.low, h.high])
            case let .setRelativePrintPosition(distance): let d = UInt16(distance)
                return .init([.ESC, .BKSL, d.low, d.high])
            case let .justification(justification): return .init([.ESC, .a, justification.rawValue])
            case let .setAbsoluteVerticalPrintPositionInPageMode(amount): return .init([.GS, .DLLR, amount.low, amount.high])
            case let .setLeftMargin(amount): return .init([.GS, .L, amount.low, amount.high])
            case let .setPrintPositionToTheBeginningOfPrintLine(flush): return .init([.GS, .T, flush ? 1 : 0])
            case let .setPrintAreaWidth(amount): return .init([.GS, .W, amount.low, amount.high])
